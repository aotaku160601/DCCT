...
チェックポイントは1個あたり約22MB（モデル本体＋Adamの状態）。全エポック分を残すと
20エポック×2モデルで1GB近くなるため、既定では**直近3個＋`best.pt` だけを残す**
（`train.keep_last_checkpoints` で変更、0で無制限）。

保存は一時ファイルに書いてから置き換えるので、途中で失敗しても既存の `best.pt` は
壊れない。保存に失敗しても学習自体は継続し、ログに空き容量つきで警告が出る。
学習開始時にも空きが2GBを切っていれば警告する。

メタデータDBはGenImage全量（約235万枚）で1GB前後になる。`db/`、`checkpoints/`、
`logs/` の使用量は次で確認できる。

```bash
df -h /
du -sh db checkpoints logs
```
...
| `unexpected pos ... vs ...` / `ios_base::clear` | チェックポイント保存中にディスクが埋まっている。`df -h /` で空きを確認する |
